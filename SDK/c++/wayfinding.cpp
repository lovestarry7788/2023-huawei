...
std::vector<double> WayFinding::joint_obs_x0_, joint_obs_y0_;
...
double WayFinding::CalcRouteDistance(Route route) {
    double sum = 0.0;
    Point s = route.front(); route.erase(route.begin());
    for(const auto u: route) {
        sum += DistBetweenPoints(s, u);
        s = u;
    }
    return sum;
}
...
//                workbench_pos.push_back({px, py});
    ...
    for (int i = 0; i < map_size_; i++) {
        for (int j = 0; j < map_size_; j++) if(map_[i][j] == '#'){
            ...
            joint_obs_[idx].push_back(py + 0.25);
            joint_obs_[idx].push_back(py - 0.25);
//            Log::print("obstacle_i : ", i, "obstacle_j : ", j, "px: ", px + 0.25, "py: ", py + 0.25);
//            Log::print("obstacle_i : ", i, "obstacle_j : ", j, "px: ", px + 0.25, "py: ", py - 0.25);
//            Log::print("obstacle_i : ", i, "obstacle_j : ", j, "px: ", px - 0.25, "py: ", py + 0.25);
//            Log::print("obstacle_i : ", i, "obstacle_j : ", j, "px: ", px - 0.25, "py: ", py - 0.25);
        }
    ...
    Log::print("M: ", M);
    for(int s = 0; s < M; ++s) {
        Log::print("s: ",s, "x: ", GetGraphPoint(s).x, "y: ", GetGraphPoint(s).y);
    }

    for(int o = 0; o < 2; ++o)
        for (int s = 0; s < M; s++)
            for (int t = 0; t < M; t++) {
                Log::print("o : ", o, "s : ", s, "t : ", t);
                if(!GetOnlineRoute(Radius[o], GetGraphPoint(s), GetGraphPoint(t), routes_[o][s][t])) routes_[o][s][t].clear(); // 如果中间有障碍，不能二分，一定要把所有的路径都清掉。
                else {
                    dist[o][s][t] = CalcRouteDistance(routes_[o][s][t]); // 能从 s -> t 计算所有路径的点的距离
//                    Log::print("o : ", o, "s : ", s, "t : ", t, "routes.size: ",routes_[o][s][t].size());
                }
            }
...
bool WayFinding::Check_Avoid_Crash_Wall(double radius, Point s, Point t) {
    double x0, x1, y0, y1;
    int x0_, x1_, y0_, y1_;
    double d;
    auto a = s;
    auto b = t; // 找到 a, b 之间的路径
    bool valid = true;
    double mind = 1e18;
...
bool WayFinding::GetOnlineRoute(double radius, Point S, Point T, Route& route) {
    //有没有装东西， 起点， 终点
    Point s = S, t = T;
    route.push_back(s);
    if(DistBetweenPoints(s, t) < 1e-3) return true; // 已经到达，返回 true
    Point res = {-1, -1};
    while(Length(t-s) > 1e-3) {
        Point mid = (s + t) / 2;
        if(Check_Avoid_Crash_Wall(radius, s, mid)) res = mid, s = mid;
        else t = mid;
    }
    Log::print("GetOnlineRoute, radius: ",radius, "s.x: ",S.x,"s.y: ",S.y,"t.x: ",T.x, "t.y: ",T.y, "res.x: ",res.x, "res.y: ",res.y);
    if(res.x == -1 && res.y == -1)  return false;
    else {
        if(!GetOnlineRoute(radius, res, T, route)) return false;
    }
    return true;
}
